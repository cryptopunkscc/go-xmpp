...
func (s *session) JID() xmpp.JID {
	return s.conn.JID()
}

// Write writes a stanza to the XMPP stream
func (s *session) Write(stanza xmpp.Stanza) error {
	s.injectID(stanza)
...
func (s *session) applyFilters(stanza xmpp.Stanza) error {
...
func (s *session) injectID(stanza xmpp.Stanza) string {
	if stanza.GetID() == "" {
